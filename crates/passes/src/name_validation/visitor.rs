...
impl NameValidationVisitor<'_> {
    ...
    /// Reject names that start with `_` at positions where the name reaches the Aleo VM.
    /// snarkVM requires identifiers to start with a letter, so a leading `_` would only
